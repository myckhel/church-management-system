...
if (empty($results)) {
    return ['status' => 0, 'message' => 'There was an error processing your request. Error Code: 002'];
}
...
if (hash_equals($calc, $auth_token->token)) {
    ...
    if (false === $user) {
        return ['status' => 0, 'message' => 'There was an error processing your request. Error Code: 003'];
    }
    ...
    if ($update == true) {
        ...
        return ['status' => 1, 'message' => 'Password updated successfully. <a href="index.php">Login here</a>'];
    }
}
?>
